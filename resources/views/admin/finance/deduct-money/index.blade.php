    ...
    <tr>
        ...
        <td>
            ...
            <button class="btn btn-success btn-xs agree"
                data-url="{{ route('admin.finance.deduct-money.agree', $data->id) }}">审核</button>
            <button class="btn btn-danger btn-xs refuse"
                data-url="{{ route('admin.finance.deduct-money.refuse', $data->id) }}">拒绝</button>
            @else
            --
            @endif
        </td>
...
<script>
    ...
    $('#submit-data-form').click(function () {
        buer_post("{{ route('admin.finance.deduct-money.store') }}", {
            user_id: $('#user-id').val(),
            fee: $('#fee').val(),
            remark: $('#remark').val()
        });
