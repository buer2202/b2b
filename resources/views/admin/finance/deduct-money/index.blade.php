...
<form class="form-inline">
    ...
    &nbsp;
    <button type="button" class="btn btn-success" id="add-new" data-toggle="modal" data-target="#myModal">创建扣款单</button>
</form>
...
<table class="table table-striped table-condensed m-t">
    ...
    @foreach ($dataList as $data)
        <tr>
            ...
            <td>
                @if ($data->status == 1)
                    <button class="btn btn-success btn-xs agree" data-url="{{ route('admin.finance.deduct-money.agree', $data->id) }}">审核</button>
                    <button class="btn btn-danger btn-xs refuse" data-url="{{ route('admin.finance.deduct-money.refuse', $data->id) }}">拒绝</button>
                @else
                    --
                @endif
            </td>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel">创建扣款单</h4>
            </div>
            <div class="modal-body">
                <form class="form-horizontal" id="data-form" method="">
                    ...
                    <div class="form-group">
                        <label for="fee" class="col-sm-2 control-label">扣款金额</label>
                        <div class="col-sm-10">
...
$('#submit-data-form').click(function () {
    ...
    $.ajax({
        url: "{{ route('admin.finance.deduct-money.store') }}",
        type: 'POST',
        dataType: 'json',
        data: {
            user_id: $('#user-id').val(),
