...
<form class="form-inline" id="data-form">
    ...
    <div class="form-group">
        <select class="form-control" name="trade_type">
            ...
            @foreach ($assetTradeType['type'] as $key => $value)
    ...
    <div class="form-group">
        <select class="form-control" name="trade_subtype">
            ...
            @foreach ($assetTradeType['sub_type'] as $key => $value)
    ...
    <tbody>
        @foreach ($dataList as $data)
            <tr>
                ...
                <td>{{ $assetTradeType['type'][$data->trade_type] ?? $data->trade_type }}</td>
                <td>{{ $assetTradeType['sub_type'][$data->trade_subtype] ?? $data->trade_subtype }}</td>
                <td>{{ $data->trade_no }}</td>
                <td>{{ $data->fee + 0}}</td>
                <td>{{ $data->remark}}</td>
                <td>{{ $data->amount + 0}}</td>
    ...
    $.get($(this).data('url'), function (data) {
        if (data.status == 1) {
            ...
            for (var i = 0; i < data.contents.length; i++) {
                html += '<tr>';
                html += '<td>' + data.contents[i].column + '</td>';
                html += '<td>' + data.contents[i].value + '</td>';
                html += '</tr>';
            }
...
$('#export-flow').click(function () {
    $.post("{{ route('admin.finance.platform-amount-flow.export') }}", $('#data-form').serialize(), function (data) {
        if (data.status) {
            $('#export-flow').attr('disabled', true).text('已进入任务队列，请稍后...');
        } else {
...
ws.onmessage = function (e) {
    ...
    switch (type) {
        case 'init':
            $.post("{{ route('admin.web-socket.gateway-worker.bind') }}", {client_id: data.client_id}, function (data) {}, 'json');
            break;
        case 'exporting_platform_amount_flow':
            $('#export-flow').attr('disabled', true).text('已导出 ' + data.message + ' 条');
            break;

        case 'complete_platform_amount_flow':
            $('#export-flow').attr('disabled', true).text('导出完成');
