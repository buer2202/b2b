...
<table class="table table-striped table-condensed m-t">
    ...
    @foreach ($dataList as $data)
        <tr>
            ...
            <td>
                @if ($data->status == 1)
                    <button class="btn btn-success btn-xs agree" data-url="{{ route('admin.finance.refund.agree', $data->id) }}">审核</button>
                    <button class="btn btn-danger btn-xs refuse" data-url="{{ route('admin.finance.refund.refuse', $data->id) }}">拒绝</button>
                @else
                    --
                @endif
            </td>
...
$('#submit-data-form').click(function () {
    ...
    $.ajax({
        url: "{{ route('admin.finance.refund.store') }}",
        type: 'POST',
        dataType: 'json',
        data: {
            user_id: $('#user-id').val(),
