            ...
            <div class="list-group">
                ...
                <p class="list-group-item form-inline {{ $role->status == 0 ? 'disabled' : '' }}">
                    ...
                    <span class="button-span pull-right"
                        data-update="{{ route('admin.system.role.update', $role->id) }}">
                        ...
                        <button type="button" class="btn btn-info btn-xs accredit"
                            data-rules="{{ route('admin.system.role.show', $role->id) }}"
                            data-update_rule="{{ route('admin.system.role.update-rules', $role->id) }}">授权</button>
                    </span>
                </p>
                @endforeach
                <form id="add-role-form" class="form-inline list-group-item new-role" style="display:none"
                    action="{{ route('admin.system.role.store') }}" method="post">
    ...
    $("#role .confirm").click(function () {
        ...
        buer_post($that.parent().data('update'), {
            ...
            if (data.status === 1) {
                ...
                //按钮切换
                $that.parent().find(".confirm, .cancel").hide().siblings(".edit, .accredit")
                    .show();
