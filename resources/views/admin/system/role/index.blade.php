            ...
            <div class="list-group">
                ...
                @foreach ($roleList as $role)
                    <p class="list-group-item form-inline {{ $role->status == 0 ? 'disabled' : '' }}">
                        ...
                        <span class="button-span pull-right" data-update="{{ route('admin.system.role.update', $role->id) }}">
                            <button type="button" class="btn btn-warning btn-xs edit">编辑</button>
                            <button type="button" class="btn btn-danger btn-xs off" data-status="{{ $role->status }}" style="display:none" data-tag="0">禁用</button>
                            <button type="button" class="btn btn-success btn-xs on" data-status="{{ $role->status }}" style="display:none" data-tag="1">启用</button>
                            ...
                            <button type="button" class="btn btn-info btn-xs accredit" data-rules="{{ route('admin.system.role.show', $role->id) }}" data-update_rule="{{ route('admin.system.role.update-rules', $role->id) }}">授权</button>
                        </span>
                    </p>
                @endforeach
                <form id="add-role-form" class="form-inline list-group-item new-role" style="display:none" action="{{ route('admin.system.role.store') }}" method="post">
                        ...
                        <div class="list-group-item">
                            ...
                            <p style="margin-bottom:0px">
                                @foreach ($rule['detail'] as $detail)
                                    ...
                                    <label class="checkbox-inline">
                                        <input type="checkbox" class="rule-item" id="{{ $detail['id'] }}" name="rule_ids[]" value="{{ $detail['id'] }}" />
...
$("#role .edit").click(function() {
    ...
    var type = $(this).parent().siblings(".role_type").data('type');
    $(this).parent().siblings("[name='type']").val(type)
});
...
$("#role .confirm").click(function() {
    ...
    $.ajax({
        type: 'POST',
        url: $that.parent().data('update'),
        data: {
            name: $that.parent().siblings("[name='name']").val(),
            type: $that.parent().siblings("[name='type']").val(),
            _method: 'put'
        },
        dataType: 'json',
        error: function (data) {
        ...
        success: function (data) {
            ...
            if(data.status === 1) {
                ...
                $that.parent().siblings(".role_type").text(data.contents.typeName);
                $that.parent().siblings(".role_type").data('type', data.contents.type);
    ...
    $.get($that.data('rules'), function(data) {
        ...
        if(data.status === 1) {
            ...
            $(".select-all").each(function () {
                var $this = $(this);
                $this.prop('checked', $this.closest('.list-group-item').find('.rule-item').length == $this.closest('.list-group-item').find('.rule-item:checked').length);
            });
        } else {
...
$(".rule-item").click(function () {
    ...
    if(!$this.prop("checked")) {
        ...
        $this.parents(".list-group-item").find("input.select-all").prop("checked", $this.closest('.list-group-item').find('.rule-item').length == $this.closest('.list-group-item').find('.rule-item:checked').length);
    }
});
