                ...
                @foreach ($roleList as $role)
                    <p class="list-group-item form-inline {{ $role->status == 0 ? 'disabled' : '' }}">
                        ...
                        <span class="button-span pull-right" data-update="{{ route('admin.user.role.update', $role->id) }}">
                            ...
                            <button type="button" class="btn btn-info btn-xs accredit" data-rules="{{ route('admin.user.role.show', $role->id) }}" data-update_rule="{{ route('admin.user.role.update-rules', $role->id) }}">授权</button>
                        </span>
                    </p>
                @endforeach
                <form id="add-role-form" class="form-inline list-group-item new-role" style="display:none" action="{{ route('admin.user.role.store') }}" method="post">
