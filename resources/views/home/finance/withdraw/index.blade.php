...
<form class="layui-form" id="search-form">
    <div class="layui-form-item">
        <div class="layui-input-inline" style="width: 100px;">
            <input type="text" class="layui-input" id="time-start" name="time_start"
                value="{{ Request::input('time_start') }}" placeholder="开始时间">
        </div>
        <div class="layui-form-mid">-</div>
        <div class="layui-input-inline" style="width: 100px;">
            <input type="text" class="layui-input" id="time-end" name="time_end"
                value="{{ Request::input('time_end') }}" placeholder="结束时间">
        </div>
        <div class="layui-input-inline" style="width: 100px;">
            <select name="status">
                <option value="">全部状态</option>
                @foreach ($assetWithdrawStatus as $key => $value)
                <option value="{{ $key }}" {{ $key == Request::input('status') ? 'selected' : '' }}>{{ $key }}.
                    {{ $value }}</option>
                @endforeach
            </select>
        </div>
...
<div id="add-window" class="layui-form" style="padding:20px 50px 0 0; display: none;">
    ...
    <div style="margin-left: 20px;">
        您尚未设置结算账号，请设置后再提现。
        <a type="button" class="layui-btn layui-btn-normal layui-btn-xs"
            href="{{ route('home.finance.settlement-account.index') }}">去设置</a>
    </div>
    @endif
</div>
@endsection
...
<script>
    layui.use(['laydate', 'form'], function () {
        ...
        $('#add-submit').click(function () {
            buer_post("{{ route('home.finance.withdraw.store') }}", {
                account_id: $('#account-id').val(),
                fee: $('#fee').val()
            }, function (data, load) {
                layer.close(load);
                if (data.status == 1) {
                    layer.alert('提现申请成功，工作人员将在T+1个工作日内审核', {
                        icon: 6
                    }, function () {
